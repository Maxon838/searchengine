...
public interface IndexRepository extends JpaRepository<IndexEntity, Integer> {
    ...
    @Query(value = "SELECT `rank_field` FROM index_table WHERE `page_id` = :pageId AND lemma_id IN :lemmasIdsList", nativeQuery = true )
    Double findRankByPageIdAndLemmaId(@Param("lemmasIdsList") List<Integer> lemmasIdsList, @Param("pageId") int pageId);
}
